...
The plan maps that hostname once per sandbox: to shared loopback for chroot, to the administrator-managed Apple `container` localhost DNS entry, to `host-gateway` for Docker Desktop and compatible Podman modes, or to the verified bridge address for local Linux bridge runtimes. Managed `socat` listeners on Linux remain separate by port while sharing the bridge address and hostname. Firewall policy retains a separate hostname/port entry for every enabled service. If two services request the same host port, planning must reject the collision rather than silently route both to the wrong upstream.
...
Run semantic CLI conflicts before secret lookup, runtime inspection, filesystem rendering, or network preflight. Run preflight against the original host-loopback address only on real runs and before starting owned forwarding resources or the sandbox. A failed connection names the configured listener and points to `internet-proxy-locally`; it never attempts repair.
