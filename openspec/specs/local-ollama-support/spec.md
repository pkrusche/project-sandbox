...
Allow the Pi agent to reach a local Ollama server running on the host, opted into via a dedicated flag, with firewall access scoped to the Ollama port and provider configuration baked into the container.
