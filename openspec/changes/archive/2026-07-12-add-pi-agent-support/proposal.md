...
- Mount Pi credentials (`~/.pi/agent/auth.json`, mode 0600) read-only into the container at `/project-sandbox-secrets/pi`, following the Codex pattern (`_sync_generic_credentials(..., include_files=("auth.json",))`) since Pi's credential shape is one flat file, not a directory tree like OpenCode's.
