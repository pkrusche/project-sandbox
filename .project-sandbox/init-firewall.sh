...
NAT6=""
[ "$IPV6_FW" = "1" ] && [ -n "$DNS6" ] && NAT6=$(ip6tables-save -t nat 2>/dev/null | grep "$DNS6" || true)
...
[ "$IPV6_FW" = "1" ] && [ -n "$NAT6" ] && echo "$NAT6" | ip6tables-restore --noflush -t nat 2>/dev/null || true
