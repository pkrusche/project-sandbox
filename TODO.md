...
- **Use safe Markdown fences in generated transcripts.**
  `transcript.py` writes tool output inside fixed triple-backtick fences. Tool
  output containing ``` can break out and inject misleading Markdown/HTML into
  the sidecar transcript. Choose a delimiter longer than any backtick run, or
  indent/escape tool output; add a fence-breaking regression test.
