...
- **Runtime networking adapter**: prefer each runtime's native loopback-forwarding mechanism (preconfigured Apple `container` localhost DNS, Docker Desktop's host alias, and Podman's `host.containers.internal`/`pasta` path). For local Linux bridge runtimes without native loopback forwarding, use a managed `socat` proxy bound only to the bridge address. Never fall back to `0.0.0.0`; fail clearly when no safe path can be verified.
...
- Update docs (`docs/usage.md`, `docs/security.md`, `docs/runtime.md`) to describe the runtime matrix, the one-time administrator command users must run themselves for Apple `container`, conditional `socat` use, lifecycle, and residual private-network exposure.
