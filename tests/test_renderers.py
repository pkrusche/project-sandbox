...
class RendererTests(TestCase):
    ...
    def test_launcher_shell_quotes_paths_with_spaces(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("target=/project-sandbox-config/claude,readonly", text)
            self.assertIn("target=/project-sandbox-config/codex,readonly", text)
            self.assertNotIn("target=/home/agent/.claude/settings.json,readonly", text)
