...
def render_pi_markdown(events: list[dict]) -> str:
    parts: list[str] = ["# Pi session transcript", ""]
    # Pi reports a failed turn only through the last assistant message, so the
    # result section is rendered from whatever the final one says.
    final_assistant: dict = {}
    for event in events:
        etype = event.get("type")
        if etype == "session":
            parts.extend(_render_pi_header(event))
        elif etype == "message_end":
            message = event.get("message")
            if not isinstance(message, dict) or message.get("role") != "assistant":
                continue
            final_assistant = message
            parts.extend(_render_pi_assistant(message))
        elif etype == "tool_execution_start":
            parts.extend(
                _render_tool_use(
                    str(event.get("toolName", "tool")), event.get("args", {})
                )
            )
        elif etype == "tool_execution_end":
            label = "error" if event.get("isError") else "result"
            result = _truncate(_tool_result_text(event.get("result")))
            parts.extend(
                [
                    f"### ↳ {event.get('toolName', 'tool')} {label}",
                    "",
                    *_code_block(result),
                    "",
                ]
            )
    parts.extend(_render_pi_result(final_assistant))
    text = "\n".join(parts)
    while "\n\n\n" in text:
        text = text.replace("\n\n\n", "\n\n")
    return text.strip() + "\n"
...
def _render_pi_header(event: dict) -> list[str]:
    rows = []
    if event.get("id"):
...
def _render_pi_assistant(message: dict) -> list[str]:
    out: list[str] = []
    content = message.get("content")
    for block in content if isinstance(content, list) else []:
        if not isinstance(block, dict):
            continue
        thinking = block.get("type") == "thinking"
        value = block.get("thinking") if thinking else block.get("text")
        if not isinstance(value, str) or not value.strip():
            continue
        label = "Assistant (thinking)" if thinking else "Assistant"
        rendered = _blockquote(value.strip()) if thinking else value.strip()
        out.extend([f"## {label}", "", rendered, ""])
    return out


def _render_pi_result(message: dict) -> list[str]:
    stop_reason = message.get("stopReason")
    if stop_reason not in ("error", "aborted"):
        return []
    out = ["---", "", "## Result", "", f"- **Status:** {stop_reason}", ""]
    error = message.get("errorMessage")
    if isinstance(error, str) and error.strip():
        out.extend([error.strip(), ""])
    return out
